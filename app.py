...
st.set_page_config(page_title="Colab Data Pipeline", layout="wide")
...
with tabs[0]:
    ...
    f_type = st.radio("ファイル形式", ["CSV", "Excel"], key="f_type_1")
    f_name = st.text_input("ファイル名", "data.csv", key="f_name_1")
    if st.button("🚀 コード生成", key="btn_1"):
...
with tabs[1]:
    ...
    outlier = st.checkbox("IQR法で異常値フラグ(is_outlier)を付与", True, key="outlier_2")
    if st.button("🚀 コード生成", key="btn_2"):
...
with tabs[2]:
    ...
    logic = st.selectbox("ロジック", ["列A * 列B", "日付から曜日抽出", "文字列検索フラグ"], key="logic_25")
    
    code_25 = ""
    if logic == "列A * 列B":
        c1 = st.text_input("カラムA", key="c1_25")
        c2 = st.text_input("カラムB", key="c2_25")
...
with tabs[3]:
    ...
    charts = st.multiselect("グラフ種類", ["散布図", "箱ひげ図", "棒グラフ", "折れ線グラフ"], ["散布図"], key="charts_3")
    if st.button("🚀 コード生成", key="btn_3"):
...
with tabs[4]:
    ...
    if st.button("🚀 コード生成", key="btn_4"):
        if task == "統計検定":
            g = st.text_input("グループ列", key="g_4")
            v = st.text_input("数値列", key="v_4")
